...
namespace {
    class CQwoynAddressVisitor : public boost::static_visitor<bool> {
    private:
        CQwoynAddress *addr;
    public:
        CQwoynAddressVisitor(CQwoynAddress *addrIn) : addr(addrIn) { }
...
bool CQwoynAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CQwoynAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CQwoynAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CQwoynAddressVisitor(this), dest);
}

bool CQwoynAddress::IsValid() const {
...
CTxDestination CQwoynAddress::Get() const {
    if (!IsValid())
...
bool CQwoynAddress::GetKeyID(CKeyID &keyID) const {
...
bool CQwoynAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CQwoynSecret::SetKey(const CKey& vchSecret) {
...
CKey CQwoynSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CQwoynSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CQwoynSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CQwoynSecret::SetString(const std::string& strSecret) {
